...
namespace GELib.Graphics.Generic_Instancing.Force
{
    ...
    //that is from the game code, you add a force to the grid by
	//
	//-	Picking a Force_Draw_Handle Handle from a "store" (an array of recyclable objects implementing the Storable interface accessed via a stack)
    //- configuring the Force_Draw_Handle using the Set_Force_Handle function
    //- adding the Force_Draw_Handle to the Spring_Grid_Force_Placement_Manager's dynamic array (in C# they're called List)
	...
	//The procedure above is similarly used in other instanced draw calls (for example placing distortions on the rendered image)
    ...
    public class Force_Draw_Handle : Render_Handle<Force_Draw_Instance>, Storable
    {
        ...
        //all 4 are also repeated in the Instance struct that gets sent via a vertex buffer to the GPU. See the Spring_Draw_Forces_I hlsl effect or the Force_Draw_Instance struct for details
        ...
        //An id specifying in which of the batches (slots) that the generic instancing template draws
        ...
        //from Storable: A stack the object should be pushed to when recycled, (to be specific when "suspended" see below or in the Storable interface comments)
        ...
        /// <summary>
        /// Called when adding a handle to the handles list, to set it after it's been retrieved from a stack containing recycled objects.
		///	The deallocation is discussed in more detail in Spring_Grid_Force_Placement_Manager
        /// </summary>
        
        public void Set_Force_Handle(Vector2 Position, float Halfaxis, float Strength, Vector2 Direction)
        {
            //The hlsl script uses normalized coordinates and scaling is a cheap way to get there from the world coords the code placing
            //the handle is using
        ...
        // The template provides the Instance arrays where the handle is supposed to put instances to, based on the handle's slot code
        // The second argument is the place where the handle should add the amount of instances it actually produced
        // (this could be refactored better, and insert_instances is a better name)
		// Basically this is like a call back that asks this request to do what it needs and provides it with where to output it.
        public void Render(
