...
//this shader is supposed to for each such part, apply its forces to all the point masses currently on it
...
//to a mass point (via a full screen quad) and then reads both the mass point's position and the Force_Spatial_Accumulator surface 
...
float4 Gather_Forces(VS_Out Input) : COLOR0
{
 ...
 //read the force placed on this area by the accumulator
